		...
		<Container maxWidth='xl'>
			<Stack mt={4} textAlign='center'>
				...
				<Typography
					...
					color='text.primary'
					mt={3}>
					خدمات جامع در زمینه کامپیوتر، نرم‌افزار و سخت‌افزار را با بالاترین سطح کیفیت به مشتریان ارائه می‌دهد. این شرکت با به‌کارگیری جدیدترین فناوری‌ها و ارائه راهکارهای نوین				</Typography>

				<Stack mt={{xs:2, lg:0}} mb={{xs:4 ,lg:7}} gap={1} direction='row' justifyContent='center'>
					{['همه', 'هوش ', 'کریپتو', 'تحصیل', 'کد ', 'دیزاین'].map((label) => (
						...
						>
							<Typography color={activeButton === label ? '#32A5D8' : 'text.primary'}>{label}</Typography>
						</CustomButton>
					))}
				</Stack>
				...
				<Grid className={classes.section} mt={{xs:2,lg:3}}>
					{AllBlogList.slice(2).map((item: AllBlogItem, index) => (
						<Grid key={item.id}>
