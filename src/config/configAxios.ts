...
const getBaseApi = process.env.NEXT_PUBLIC_BASE_URL;
    ...
    (err) => {
      ...
      if (err?.response?.status === 401 && !originalRequest._retry) {
        ...
        return new Promise((resolve, reject) => {
          const { token } = store.getState().user;
          fetch(
            `${process.env.REACT_APP_BASE_API}auth/v1.0/token/refresh/${
      ...
      // TODO show translate ErrorMessage => message: ErrorMessageHandler(originalRequestErrorCode)
